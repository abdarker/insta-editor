...
const MainContent: React.FC = () => {
  ...
  const [tabValue, setTabValue] = useState<string>("insta-filter");
  ...
  const handleSepia = async (event: React.ChangeEvent<HTMLInputElement>) => {
    setSepia(parseInt(event.target.value, 10));
  };
  const handleGrayScale = async (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    setGrayScale(parseInt(event.target.value, 10));
  };
      ...
      <div className="flex flex-col gap-4 pt-4">
        ...
        {selectedImage && (
          <div ref={imgResultRef} className="md:w-1/2 md:h-1/2">
            <img
              ...
              alt="Selected Image"
              style={
                tabValue == "custom-filter"
                  ? {
                      filter: `contrast(${contrast}%) brightness(${brightness}%) saturate(${saturation}%) sepia(${sepia}%) grayScale(${grayScale}%) `,
                    }
                  : {}
              }
            />
        ...
        <div>
          ...
          >
            <TabsList className="w-full mb-2">
              <TabsTrigger value="insta-filter" className="w-full">
                Instagram Filter
              </TabsTrigger>
              <TabsTrigger value="custom-filter" className="w-full">
                Custom Filter
              </TabsTrigger>
            </TabsList>
            {selectedImage && (
            ...
            <TabsContent value="insta-filter" >
              <div>
            ...
            <TabsContent value="custom-filter">
              <div className="flex flex-col gap-4">
                ...
                <Slider
                  label="Saturation"
                  value={saturation}
                  handleRangeChange={handleSaturation}
                />
                <Slider
                  label="Sepia"
                  value={sepia}
                  handleRangeChange={handleSepia}
                  oneSided
                />
                <Slider
                  oneSided
                  label="Gray Scale"
                  value={grayScale}
                  handleRangeChange={handleGrayScale}
                />
              </div>
            </TabsContent>
          </Tabs>
        ...
        <div className="flex w-full items-center justify-center space-x-4 pt-4">
          <a
            ...
            href="https://github.com/abdarker/insta-editor"
          >
        ...
        {visibility && (
          <div className=" flex flex-col items-center justify-center">
            😂😂😂😂
            <img
              src="https://c.tenor.com/x8v1oNUOmg4AAAAC/tenor.gif"
              alt="rick roll"
